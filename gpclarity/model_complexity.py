...
class ComplexityMetrics:
    ...
    def risk_factors(self) -> List[str]:
        ...
        if self.category in (
            ...
            ComplexityCategory.TOO_COMPLEX,
        ):
...
class ComplexityAnalyzer:
    ...
    def _geometric_score(model: Any, X: np.ndarray, **kwargs) -> float:
        ...
        try:
            ...
            if np.sum(eigenvals) > 0:
                effective_rank = (np.sum(eigenvals) ** 2) / (
                    np.sum(eigenvals**2) + 1e-10
                )
    ...
    def analyze(self, model: Any, X: np.ndarray) -> ComplexityMetrics:
        ...
        score_fn = self._strategies.get(self.scoring_strategy, self._default_score)
    ...
    def _compute_effective_dof(self, model: Any, X: np.ndarray) -> float:
        ...
        try:
            ...
            noise_var = 1.0
            if hasattr(model, "Gaussian_noise") and hasattr(
